...
import { Share2, Home, RotateCcw, BarChart3, Trophy } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function ResultModal({ open, onClose, win, modeLen, guesses, durationMs, grid, solution, lastGuess, xp, coins, modeType = "daily", unlockedAchievements = [] }: ResultModalProps) {
  ...
  const shareText = `Günlük Kelime ${modeLen} • ${win ? "Kazandım" : "Kaybettim"} • ${guesses}/${6}${seconds ? ` • ${seconds}s` : ""}\n\n${grid}\n\nhttps://gizlikelime.local`;
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md bg-white border-[#e8e0d5] text-[#4a4a4a] shadow-soft rounded-2xl overflow-y-auto max-h-[90vh]">
        <DialogHeader>
          <DialogTitle className="text-[#4a4a4a] text-xl font-bold">{win ? "🎉 Tebrikler!" : "😅 Sonraki sefere"}</DialogTitle>
          <DialogDescription className="text-[#9a9a9a]">
            {modeLen} harfli mod • {guesses}. tahminde {win ? "kazandın" : "bilemedin"}
            {seconds ? ` • ${seconds}s` : ""}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-2">
            ...
            {!win && lastGuess && (
              <div className="flex items-center justify-between rounded-xl bg-[#f5c6d6]/20 border border-[#f5c6d6] px-4 py-3">
                <span className="text-sm font-semibold text-[#d88080]">Son Tahmin:</span>
